#!/usr/bin/env node
/**
 * Import NCD Indicator Catalog from Excel into Supabase sheet tables.
 *
 * Usage:
 *   node scripts/import-ncd-sheets.js > /tmp/import-ncd-sheets.sql
 *
...
function main() {
  ...
  sql.push("  sheet_who_id UUID;");
  sql.push("  sheet_hearts_id UUID;");
  sql.push("  sheet_diabetes_id UUID;");
  sql.push("  sheet_cnss_id UUID;");
  sql.push("BEGIN");
  ...
  sql.push("  INSERT INTO sheet_groups (id, name, description, color, sort_order, created_by)");
  sql.push("  VALUES (");
  ...
  // Define sheets
  const sheetDefs = [
    { sheet: 'WHO NCD Progress', name: 'WHO NCD Progress', varName: 'sheet_who_id',
    ...
    { sheet: 'HEARTS', name: 'HEARTS', varName: 'sheet_hearts_id',
    ...
    { sheet: 'Diabetes Monitoring', name: 'Diabetes Monitoring', varName: 'sheet_diabetes_id',
    ...
    { sheet: 'CNSS', name: 'CNSS', varName: 'sheet_cnss_id',
  ...
  for (const def of sheetDefs) {
    ...
    sql.push(`  -- Sheet: ${def.name}`);
    sql.push(`  INSERT INTO sheets (id, name, description, icon, columns, sort_order, group_id, created_by)`);
    sql.push(`  VALUES (`);
    sql.push(`    gen_random_uuid(),`);
    ...
    sql.push(`  INSERT INTO sheet_rows (sheet_id, cells, sort_order, created_by) VALUES`);
