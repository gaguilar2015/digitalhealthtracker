...
import type { Sheet, SheetRow, SheetRowGroup } from '@/types';

export function exportSheetToExcel(sheet: Sheet, rows: SheetRow[]) {
  ...
  if (isGrouped) {
    const sections = buildGroupSections(sheet, rows, isAutoGroup, manualGroups);
    for (const section of sections) {
      // Group header row
      sheetData.push([section.name, ...Array(headers.length - 1).fill('')]);
      for (const row of section.rows) {
        sheetData.push(buildRowData(sheet, row));
      }
    }
  } else {
  ...
  if (isGrouped) {
    ...
    for (const section of sections) {
      ...
      if (!ws[cellRef]) ws[cellRef] = { t: 's', v: section.name };
  ...
  const fileName = `${sheet.name.replace(/[^a-zA-Z0-9]/g, '_')}_${format(new Date(), 'yyyy-MM-dd')}.xlsx`;
  XLSX.writeFile(wb, fileName);
}

function buildRowData(sheet: Sheet, row: SheetRow): string[] {
...
function buildGroupSections(
  ...
  rows: SheetRow[],
  isAutoGroup: boolean,
  manualGroups: SheetRowGroup[],
): GroupSection[] {
  if (isAutoGroup && sheet.group_by_column) {
    const colId = sheet.group_by_column;
