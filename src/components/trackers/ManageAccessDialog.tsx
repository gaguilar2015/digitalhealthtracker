...
import type { TrackerMember, TrackerGroupMember, TeamMember } from '@/types';

interface ManageAccessDialogProps {
  open: boolean;
  onClose: () => void;
  creatorId: string;
  trackerMembers: TrackerMember[];
  teamMembers: TeamMember[];
  onAddMember: (teamMemberId: string) => Promise<void>;
  onRemoveMember: (trackerMemberId: string) => Promise<void>;
  groupName?: string;
  groupColor?: string;
  groupMembers?: TrackerGroupMember[];
}

export function ManageAccessDialog({
  open, onClose, creatorId,
  trackerMembers, teamMembers,
  ...
  const memberIds = useMemo(
    () => new Set(trackerMembers.map(m => m.team_member_id)),
    [trackerMembers],
  );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-surface-200 max-w-md w-full p-6 max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-2">
          {trackerMembers.map(tm => {
            const member = teamMembers.find(m => m.id === tm.team_member_id);
            if (!member) return null;
            const isCreator = tm.team_member_id === creatorId;
