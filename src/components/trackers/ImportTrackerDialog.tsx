...
import type { TrackerColumn, TrackerColumnType } from '@/types';
import {
  parseSpreadsheetFile,
  getSheetData,
  inferColumns,
  convertCellValue,
} from '@/lib/utils/importTracker';
import type XLSX from 'xlsx';

interface ImportTrackerDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (data: {
    name: string;
...
export function ImportTrackerDialog({ open, onClose, onSubmit }: ImportTrackerDialogProps) {
  ...
  const handleColumnChange = (index: number, field: 'name' | 'type', value: string) => {
    setColumns(prev => prev.map((col, i) => {
      ...
      if (field === 'type') {
        const newCol = { ...col, type: value as TrackerColumnType };
        if (value === 'select') {
  ...
  const handleImport = async () => {
    if (!name.trim()) {
      setImportError('Tracker name is required.');
      return;
    }
    if (columns.length === 0) {
      ...
      <div className="relative bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-surface-200 max-w-2xl w-full p-6 max-h-[90vh] overflow-y-auto">
        ...
        {step === 'importing' && (
          <div className="flex flex-col items-center gap-3 py-8">
            <Loader2 className="w-8 h-8 text-primary-500 animate-spin" />
            <p className="text-sm text-gray-600">Creating tracker and importing rows...</p>
          </div>
        )}
      </div>
    </div>
