...
interface EditSheetDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (data: EditSheetFormValues & { icon: string | null; group_id: string | null }) => Promise<void>;
  sheet: Sheet;
}

export function EditSheetDialog({ open, onClose, onSubmit, sheet }: EditSheetDialogProps) {
  const [selectedIcon, setSelectedIcon] = useState<string>(sheet.icon ?? 'Table2');
  const [selectedGroupId, setSelectedGroupId] = useState<string | null>(sheet.group_id);
  ...
  const {
    ...
    resolver: zodResolver(editSheetSchema) as any,
    defaultValues: {
      name: sheet.name,
  ...
  useEffect(() => {
    if (open) {
      ...
      setSelectedIcon(sheet.icon ?? 'Table2');
      setSelectedGroupId(sheet.group_id);
    }
  }, [open, sheet, reset]);

  const handleFormSubmit = async (data: EditSheetFormValues) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-surface-200 max-w-md w-full p-6">
        ...
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Edit Sheet</h3>
