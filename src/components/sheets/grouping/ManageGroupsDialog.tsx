...
import type { Sheet, SheetColumn, SheetRowGroup } from '@/types';
...
interface ManageGroupsDialogProps {
  ...
  sheet: Sheet;
  columns: SheetColumn[];
  onSave: (data: { row_groups?: SheetRowGroup[]; group_by_column?: string | null; auto_group_order?: string[] }) => Promise<void>;
}

export function ManageGroupsDialog({ open, onClose, sheet, columns, onSave }: ManageGroupsDialogProps) {
  const [mode, setMode] = useState<'manual' | 'auto'>('manual');
  const [groups, setGroups] = useState<SheetRowGroup[]>([]);
  const [autoColumn, setAutoColumn] = useState<string | null>(null);
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    if (open) {
      const hasAuto = !!sheet.group_by_column;
      setMode(hasAuto ? 'auto' : 'manual');
      setGroups((sheet.row_groups ?? []).map(g => ({ ...g })));
      setAutoColumn(sheet.group_by_column ?? null);
    }
  }, [open, sheet]);
  ...
  const updateGroup = (index: number, updates: Partial<SheetRowGroup>) => {
