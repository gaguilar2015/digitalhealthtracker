...
export function ManageColumnsDialog({ open, onClose, columns, onSave }: ManageColumnsDialogProps) {
  ...
  const [conditionalCol, setConditionalCol] = useState<SheetColumn | null>(null);
  ...
  const updateCol = (index: number, updates: Partial<SheetColumn>) => {
  ...
  const handleConditionalSave = (col: SheetColumn) => {
                ...
                <div className="flex-1 space-y-2">
                  <div className="flex gap-2">
                    ...
                    <select
                      value={col.type}
                      onChange={e => {
                        const newType = e.target.value as SheetColumn['type'];
                        const updates: Partial<SheetColumn> = { type: newType };
