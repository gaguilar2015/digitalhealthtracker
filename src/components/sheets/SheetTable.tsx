...
import { SortableSheetRow } from './SortableSheetRow';
import { resolveEffectiveStyle } from '@/lib/utils/cellFormatting';
import type { SheetColumn, SheetRow, Sheet, CellStyle, SheetRowGroup } from '@/types';
...
export function SheetTable({
  ...
  const [localRows, setLocalRows] = useState<SheetRow[]>(rows);
  ...
  const isAutoGroup = !!sheet.group_by_column;
  const hasManualGroups = (sheet.row_groups ?? []).length > 0 && !isAutoGroup;
  ...
  const sortRows = useCallback((input: SheetRow[]) => {
    if (!sortCol || !sortDir) {
  ...
  const groupedSections = useMemo((): GroupedSection[] => {
    const groupByCol = sheet.group_by_column;
    const manualGroups = sheet.row_groups ?? [];
    ...
    if (isAutoGroup) {
      ...
      const persistedOrder = sheet.auto_group_order ?? [];
  ...
  }, [sheet, columns, localRows, sortRows, isAutoGroup, hasManualGroups]);
  ...
  const handleDragEnd = useCallback((event: DragEndEvent) => {
    ...
    let sectionRows: SheetRow[];
    if (hasManualGroups || isGrouped) {
      if (targetGroupId === null) {
        // Ungrouped
        const manualGroupIds = new Set((sheet.row_groups ?? []).map(g => g.id));
  ...
  }, [localRows, rows, onRowMove, findRowGroup, findGroupForOverId, hasManualGroups, isGrouped, sheet.row_groups]);
