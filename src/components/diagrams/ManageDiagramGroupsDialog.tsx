...
import { ManageDiagramGroupAccessDialog } from './ManageDiagramGroupAccessDialog';

interface ManageDiagramGroupsDialogProps {
  open: boolean;
  onClose: () => void;
}
...
export function ManageDiagramGroupsDialog({ open, onClose }: ManageDiagramGroupsDialogProps) {
  const { user } = useAuth();
  const { groups, createGroup, updateGroup, deleteGroup } = useDiagramGroups();
  ...
  const { groupMembers, addGroupMember, removeGroupMember } = useDiagramGroupMembers(accessGroupId ?? undefined);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-surface-200 max-w-lg w-full p-6 max-h-[90vh] overflow-y-auto">
        ...
        <h3 className="text-lg font-semibold text-gray-900 mb-1">Manage Diagram Groups</h3>
        <p className="text-sm text-gray-500 mb-4">
          Organize diagrams into collapsible folders in the sidebar.
        </p>
        ...
        <p className="mt-2 text-xs text-gray-400">
          Diagrams not assigned to a group will appear under &quot;Ungrouped&quot;.
        </p>
      ...
      {/* Group access dialog */}
      {accessGroup && (
        <ManageDiagramGroupAccessDialog
          open={!!accessGroupId}
          ...
          onAddMember={async (teamMemberId) => {
            await addGroupMember({ diagram_group_id: accessGroup.id, team_member_id: teamMemberId });
          }}
