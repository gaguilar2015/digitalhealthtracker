...
interface EditDiagramFormValues {
  name: string;
...
interface EditDiagramDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (data: EditDiagramFormValues & { icon: string | null; group_id: string | null }) => Promise<void>;
  diagram: Diagram;
}

export function EditDiagramDialog({ open, onClose, onSubmit, diagram }: EditDiagramDialogProps) {
  const [selectedIcon, setSelectedIcon] = useState<string>(diagram.icon ?? 'Workflow');
  const [selectedGroupId, setSelectedGroupId] = useState<string | null>(diagram.group_id);
  ...
  } = useForm<EditDiagramFormValues>({
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    resolver: zodResolver(editDiagramSchema) as any,
    defaultValues: { name: diagram.name, description: diagram.description },
  });

  useEffect(() => {
    if (open) {
      reset({ name: diagram.name, description: diagram.description });
      setSelectedIcon(diagram.icon ?? 'Workflow');
      setSelectedGroupId(diagram.group_id);
    }
  }, [open, diagram, reset]);

  const handleFormSubmit = async (data: EditDiagramFormValues) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-surface-200 max-w-md w-full p-6">
        ...
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Edit Diagram</h3>
